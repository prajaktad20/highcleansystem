...
public $user_role_base_url = ''; public $user_dashboard_url = '';
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['SwmsTask']))
		{
			$model->attributes=$_POST['SwmsTask'];
			if($model->save())
	...
	{
		$dataProvider=new CActiveDataProvider('SwmsTask');
	...
	public function actionAdmin()
	{
		...
		$model=new SwmsTask('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SwmsTask']))
			$model->attributes=$_GET['SwmsTask'];
	 ...
	 * @return SwmsTask the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=SwmsTask::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param SwmsTask $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='swms-task-form')
		{
