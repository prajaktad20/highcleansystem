...
public $user_role_base_url = ''; public $user_dashboard_url = '';
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Setting']))
		{
			$model->attributes=$_POST['Setting'];
                        $model->meta_value= trim($_POST['Setting']['meta_value']) ;
			
			 $meta_key= Setting::model()->findByAttributes(array('meta_key'=>$_POST['Setting']['meta_key']));
			if($meta_key===null) {
                           if($model->save())
                                                    $this->redirect(array('admin','id'=>$model->setting_id));
                        }else{
                                            $model->addError('meta_key', "This meta key allready exist, Please try again using another meta key.");
                                    }
	...
	public function actionAdmin()
	{
		...
		if ( isset( $_GET[ 'pageSize' ] ) )
		{
          ...
          $files = glob(Yii::app()->basePath . '/../uploads/temp/*'); // get all file names
