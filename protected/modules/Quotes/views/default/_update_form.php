...
?>					
					...
					<tr>
                      <th><?php echo $form->labelEx($model,'site_id',array('class'=>'col-sm-5')); ?><br/>
					  <small>( if you change site, your previous data related this quote will be lost )</small>
					  </th>
                      <td>
					  	<div class="createselect mr30">
...
<?php		if(isset($model->site_id) && !empty($model->site_id)) { 
         ...
         foreach ($loop_builings as $value) {
			...
			if(isset($_POST['site_buildings'])) {
			if(in_array($value->id,$site_buildings))
			$checked_yes = 'checked';
			else
			$checked_yes = '';
			} else {
			if(isset($building_ids) && in_array($value->id,$building_ids))
			$checked_yes = 'checked';
			else
			$checked_yes = '';
			}
			
			 $buildings_options .= '<input readonly '.$checked_yes.' id="building_id'.$value->id.'" type="checkbox" value="'.$value->id.'" name="site_buildings[]">&nbsp;&nbsp;' . $value->building_name.'<br/>';
           }
