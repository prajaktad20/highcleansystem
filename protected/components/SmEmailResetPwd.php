...
class SmEmailResetPwd extends CApplicationComponent {
	...
	public static function sendEmail($email_temp_id, $user_id) {
        ...
        // user details
        $user_model = StateManager::model()->findByPk($user_id);
        if ($user_model === NULL)
            return;


        /* E-mail Formatting */
        $EmailFormatting = SmEmailResetPwd::emailFormatting($email_temp_id, $user_model);
        ...
        SmEmailResetPwd::sendMailThroughMailgun($from, $fromheader, $to, $toname, $sub, $mail, $header);
    ...
    public static function emailFormatting($email_temp_id, $user_model) {
        ...
        $user_reset_link = '<a href="' . Yii::app()->getBaseUrl(true) . '/state?r=site/Resetaccount&code=' . $user_model->id . '-' . $user_model->password . '"><span style="">Reset your password</span></a>';
