...
class AgentEmailResetPwd extends CApplicationComponent {
	...
	public static function sendEmail($email_temp_id, $user_id) {
        ...
        // user details
        $user_model = Agents::model()->findByPk($user_id);
        if ($user_model === NULL)
            return;


        /* E-mail Formatting */
        $EmailFormatting = AgentEmailResetPwd::emailFormatting($email_temp_id, $user_model);
        ...
        $to = trim($user_model->business_email_address); // user email address
        $toname = ucwords(trim($user_model->agent_first_name . ' ' . $user_model->agent_last_name)); // user first name and last name
        ...
        AgentEmailResetPwd::sendMailThroughMailgun($from, $fromheader, $to, $toname, $sub, $mail, $header);
    ...
    public static function emailFormatting($email_temp_id, $user_model) {
        ...
        $user_first_name = '<span style="">' . $user_model->agent_first_name . '</span>';
        $user_last_name = '<span style="">' . $user_model->agent_last_name . '</span>';
        $user_reset_link = '<a href="' . Yii::app()->getBaseUrl(true) . '/agent?r=site/Resetaccount&code=' . $user_model->id . '-' . $user_model->password . '"><span style="">Reset your password</span></a>';
