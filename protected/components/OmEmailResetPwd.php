...
class OmEmailResetPwd extends CApplicationComponent {
	...
	public static function sendEmail($email_temp_id, $user_id) {
        ...
        // user details
        $user_model = OperationManager::model()->findByPk($user_id);
        if ($user_model === NULL)
            return;


        /* E-mail Formatting */
        $EmailFormatting = OmEmailResetPwd::emailFormatting($email_temp_id, $user_model);
        ...
        $to = trim($user_model->email_address); // user email address
        ...
        OmEmailResetPwd::sendMailThroughMailgun($from, $fromheader, $to, $toname, $sub, $mail, $header);
    ...
    public static function emailFormatting($email_temp_id, $user_model) {
        ...
        $user_reset_link = '<a href="' . Yii::app()->getBaseUrl(true) . '/operation?r=site/Resetaccount&code=' . $user_model->id . '-' . $user_model->password . '"><span style="">Reset your password</span></a>';
