...
class SoEmailResetPwd extends CApplicationComponent {
	...
	public static function sendEmail($email_temp_id, $user_id) {
        ...
        // user details
        $user_model = SystemOwner::model()->findByPk($user_id);
        if ($user_model === NULL)
            return;


        /* E-mail Formatting */
        $EmailFormatting = SoEmailResetPwd::emailFormatting($email_temp_id, $user_model);
        ...
        SoEmailResetPwd::sendMailThroughMailgun($from, $fromheader, $to, $toname, $sub, $mail, $header);
