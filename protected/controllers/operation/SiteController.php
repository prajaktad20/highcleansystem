...
class SiteController extends Controller {
    ...
    public function init() {
        ...
        $this->user_role_base_url = CommonFunctions::getUserBaseUrl('operation_manager');
    }
	...
	public function actionIndex()
	{
		...
		if(Yii::app()->user->isGuest)
                    $this->redirect(Yii::app()->baseUrl.'/operation?r=site/login');



                 if(Yii::app()->user->name =='operation_manager'  )
                             $this->redirect(Yii::app()->baseUrl.'/operation?r=OperationDashboard');
	...
	public function actionError()
	{
		    ...
		    header("Location: $base/operation/");
		    exit();
	...
	{	
		...
		if(isset($_POST['OperationLoginForm']))
		{  
			$model->attributes=$_POST['OperationLoginForm'];
			// validate user input and redirect to the previous page if valid
			if($model->validate() && $model->login()){

                            	$base = Yii::app()->baseUrl;
			if(Yii::app()->user->profile==='operation_manager')
			{	
				header("Location: $base/operation?r=OperationDashboard");
				exit();
			} 
	...
	{
              Yii::app()->user->logout();
              $this->redirect(Yii::app()->baseUrl.'/operation');
	}
...
public function actionReset() {
        ...
        if (isset($_POST['PasswordResetRequestForm'])) {
            ...
            if ($model->validate()) {
                ...
                $data_Model = OperationManager::model()->findByAttributes(array(), $condition = 'email_address = :email AND status = :status', $params = array(':email' => $email, ':status' => '1')
                );
                if ($data_Model === NULL) {
                    ...
                    OmEmailResetPwd::sendEmail(12, $user_id);
                }
            }
        }
    ...
    public function actionResetaccount() {
        ...
        if (is_array($code_part) && count($code_part) == 2) {
            ...
            $user_model_data = OperationManager::model()->findByAttributes(array(), $condition = 'id = :id AND password=:password AND status = :status', $params = array(':id' => $id, 'password' => $password, ':status' => '1'));
                ...
                if (isset($_POST['PasswordResetAccountForm'])) {
                    ...
                    if ($model->validate()) {
                        ...
                        if ($user_model_data->save()) {
                            ...
                            /// mail code
                            $this->redirect(Yii::app()->baseUrl . '/operation?r=site/ForgotThankyou&reset=1');
