...
class SiteController extends Controller {
    ...
    public function init() {
        ...
        $this->user_role_base_url = CommonFunctions::getUserBaseUrl('agent');
    }
	/**
	...
	public function actionIndex()
	{
		...
		if(Yii::app()->user->isGuest)
                    $this->redirect(Yii::app()->baseUrl.'/agent?r=site/login');



                 if(Yii::app()->user->name =='agent'  )
                             $this->redirect(Yii::app()->baseUrl.'/agent?r=AgentDashboard');
	...
	{	
		...
		{  
			...
			if($model->validate() && $model->login()){
                            	$base = Yii::app()->baseUrl;
			if(Yii::app()->user->profile==='agent')
			{		                        
				header("Location: $base/agent?r=AgentDashboard");
				exit();
			} 
...
public function actionReset() {
        ...
        if (isset($_POST['PasswordResetRequestForm'])) {
            ...
            if ($model->validate()) {
                ...
                $data_Model = Agents::model()->findByAttributes(array(), $condition = 'business_email_address = :email AND status = :status', $params = array(':email' => $email, ':status' => '1')
                );
                if ($data_Model === NULL) {
                    ...
                    AgentEmailResetPwd::sendEmail(12, $user_id);
                }
            }
        }
    ...
    public function actionResetaccount() {
        ...
        if (is_array($code_part) && count($code_part) == 2) {
            ...
            $user_model_data = Agents::model()->findByAttributes(array(), $condition = 'id = :id AND password=:password AND status = :status', $params = array(':id' => $id, 'password' => $password, ':status' => '1'));
                ...
                if (isset($_POST['PasswordResetAccountForm'])) {
                    ...
                    if ($model->validate()) {
                        ...
                        if ($user_model_data->save()) {
                            ...
                            /// mail code
                            $this->redirect(Yii::app()->baseUrl . '/agent?r=site/ForgotThankyou&reset=1');
                            EXIT();
                        }
                    }
                }
